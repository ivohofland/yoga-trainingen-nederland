...
- `scheduled_hours_ceiling` = `{kind: computed, value, working}` | `{kind: no_schedule, value: null}`. OURS on every programme — no `published` variant (no school publishes it). `Σ count × (end − start − pause_min)`, an **upper bound** on contact hours: clock time in the room ≥ contact time (i.e. contact hours ≤ this value — the mathematical relation, not a literal render). Rendered as "ten hoogste X uur". `no_schedule` where the record holds no `schedule`. Never stored.
- `hours_disconnect` = `{kind: computed, value, working}` | `{kind: no_comparison, value: null}` | `{kind: no_shortfall, value: null}`. OURS. `total_hours − scheduled_hours_ceiling` — a **lower bound** on the hours the timetable can't account for (the ceiling is an upper bound, so the gap is at least this value — the mathematical relation, not a literal render). Rendered as "minstens X uur niet ingeroosterd". `no_comparison` where there's no schedule, or no PUBLISHED total (a total WE summed is not a claim to disconnect from). `no_shortfall` where the published total is at or below the ceiling — a non-positive gap is not a finding, never rendered as a negative "minstens" figure. Never stored.
