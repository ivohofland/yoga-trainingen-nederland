...
import { YearMonth, type Program, type PublicArchive, type Provider } from "../schema";
...
/** The fields every reference fixture needs, MINUS `public_archive` — the Wayback test
 *  below needs a different value per case, and a second `public_archive:` key appended
 *  after REF_BASE's own would be a duplicate YAML key, which the parser rejects outright. */
const REF_BASE_CORE = `id: x-2026-07
title: "T"
publisher: "P"
type: other
captured: "2026-07"
...
/** `public_archive` defaults to `not_yet` — a gap, not a claim — so the fixtures below
 *  that do not care about it (the local_snapshot checks) stay schema-valid without saying
 *  more than they mean. */
const REF_BASE = `${REF_BASE_CORE}public_archive:\n  kind: not_yet\n`;
...
test("INTEGRITY: a public archive that proves nothing is not claimed as one", () => {
      ...
      s.id === "crkbo-register-2026-06"
        ? {
            ...
            public_archive: {
              ...
              url: "https://web.archive.org/web/20260612/https://www.crkbo.nl/Register/Instellingen",
            },
          }
        : s,
    ),
  ...
  for (const p of providers) {
    for (const s of p.sources) {
      if (!s.url || s.public_archive.kind !== "archived") continue;
      if (waybackIsPointless(s.url)) {
        assert.doesNotMatch(
          s.public_archive.url,
          /web\.archive\.org/i,
...
/**
 * Build a real provider with exactly one source's `url` and `public_archive` overridden —
 * spread from dru-yoga's `ya-register-2026-06` source, so every OTHER required field on
 * `Source`/`Provider` still comes from a real, schema-valid record. The same reason
 * price-gap.fixture.ts spreads a real programme rather than hand-writing a `Provider`
 * literal: a hand-written one drifts from the schema the moment a required field is added,
 * silently, because nothing re-checks a literal against the schema it was copied from.
 */
function providerWithSource(overrides: { url?: string; public_archive: PublicArchive }): Provider {
  const base = providerOf("dru-yoga");
  return {
    ...base,
    sources: base.sources.map((s) => (s.id === "ya-register-2026-06" ? { ...s, ...overrides } : s)),
  };
}
